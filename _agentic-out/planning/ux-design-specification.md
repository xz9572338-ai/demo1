...
## 首发试点 UX 基线（2026-08-17 批准）

本节是首发实现的权威 UX 范围；后文保留为增强版 1.x 的完整体验目标。首发只建设五类页面：注册、登录/入驻状态、应用与首次凭证、接口权限、静态 API 文档。

- 不建设六阶段总览、复杂应用工作区、密钥轮换面板、应用停用/恢复、页面沙箱调试器、响应诊断器、调用统计、生产准入工作台和问题工单。
- 入驻驳回页只展示原因、更新时间与企业微信/邮件支持渠道；资料修正和重新提交由线下处理。
- 每客户最多一个应用；应用页合并创建、AppID 与一次性 AppSecret 展示，不显示多应用筛选或复杂环境页签。
- 权限页固定展示三个查询接口及待审核/通过/驳回状态；审批与客户绑定由内部数据库操作完成。
- API 文档使用静态 OpenAPI 渲染页并提供 cURL、Java、Python 示例，不提供页面发请求能力。
- 首发优先支持 Chrome/Edge 最近两个主要版本的常用桌面宽度；注册、登录、状态、应用、权限和文档导航必须可用键盘完成。完整 320～1440px 矩阵、200% 缩放全场景和高级视觉回归移至 1.x。
- 错误仍必须展示稳定错误码和 Request ID（如存在）；AppSecret 不进入 URL、缓存、日志或持久化存储。

首发主路径为：注册 → 登录查看审核状态 → 审核通过后创建唯一应用并保存沙箱密钥 → 提交三项权限 → 阅读文档并在客户系统联调 → 技术负责人线下验收并开通生产。后文与此路径冲突的页面、组件和旅程均标记为 1.x 设计储备。
