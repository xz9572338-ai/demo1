...
**控制台认证：** 登录名或手机号加密码；Spring Security 服务端会话；`HttpOnly + Secure + SameSite` Cookie；Redis 统一存储会话；变更请求启用 CSRF；一期不使用 JWT。
...
**授权与隔离：** 默认拒绝；每次请求验证环境、应用状态、生产准入和逐接口权限；企业及数据范围从 AppID 绑定关系解析，不信任请求中的客户标识；内部人工审核不通过客户控制台暴露。
